...
def build(bfl):
    ...
    aper = CircularAperture(diameter=400)

    s.add_surface(300, 1, "flat", name="1", far_field=True, aperture=aper)
    
    s.add_surface(bfl, n2, "spherical", R=200, name="2", aperture=aper)
    
    s.add_surface(0, 1, "flat", name="image", aperture=aper, absorber=True)
...
for theta in np.linspace(0, 40, 5):
    f = InfiniteFan(t, theta=theta*np.pi/180, color=hsv2rgb(theta * 300/40, 1, 1), D=380)
