...
def build(bfl):
    ...
    l2 = Lens.with_shape(f2, -1.8, n2, t=100)

    print(f"Lens1: R1: {l1.R1} R2: {l1.R2} t: {l1.t} ap1: {l1.max_aperture+100}")
    print(f"Lens2: R1: {l2.R1} R2: {l2.R2} t: {l2.t}")

    print(f"Lens distance: {d}")

    
    s.add_surface(l1.t, l1.n, "spherical", R=l1.R1, name="l1-1", aperture=l1.max_aperture+100)
    ...
    s.add_surface(bfl, n2, "spherical", R=hyperR, name="2", aperture=2*hyperR)
    
    s.add_surface(0, 1, "flat", name="image", aperture=2*hyperR, absorber=True)
...
if False:
    for theta in np.array([0]) * np.pi / 180:
        f = trial_for_angle(theta)
        print(f"xa: {f(200)} xb: {f(250)} xc: {f(300)}")
    
        l = minimize_scalar(f, bracket=(100, 200, 300)) #solve_minima(trial, 200)
